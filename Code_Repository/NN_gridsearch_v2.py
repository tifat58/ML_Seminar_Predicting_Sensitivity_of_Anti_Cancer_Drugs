...
#        pickle.dump(feat, f, pickle.HIGHEST_PROTOCOL)
    ...
    summ = list()
    stats = []
    summfile = "../reports/summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layerslog5_ep500_ba30_exp.txt"
    ...
    for n_layers in range(int(sys.argv[4]), int(sys.argv[5])+1):
        ...
        dir_path = "../Figure/" + "drug_" + str(sys.argv[2]) + "_features_" + str(sys.argv[1]) + "_epoch_" + str(training_epochs) + "_nLayers_" + str(n_layers) + "_" + str(sys.argv[6])
        ...
        outputfile = "../reports/report_" + sys.argv[2] + "_" + sys.argv[1] + "_layerslog5_ep500_ba30_" + str(n_layers) + "_exp.txt"
        ...
        for i in range(1, (n_layers-1)):
            ...
            weights['w' + str(i)] = tf.Variable(tf.random_normal([int(np.around(hidden_neurons[i-1])), int(np.around(hidden_neurons[i]))]))
