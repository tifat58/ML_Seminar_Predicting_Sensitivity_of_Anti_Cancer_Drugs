...
with tf.device('/device:GPU:' + str(gpu)):
    ...
    summ = list()
    stats = []
    summfile = "../reports/summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layerslin5_ep250_ba30_exp.txt"
	...
	#hidden_neurons = np.logspace(np.log10(float(sys.argv[1])), np.log10(5.0), num=n_layers)
        ...
        dir_path = "../Figure/" + "drug_" + str(sys.argv[2]) + "_features_" + str(sys.argv[1]) + "_epoch_" + str(
            training_epochs) + "_nLayerslin_" + str(n_layers) + "_" + str(sys.argv[6])
        ...
        outputfile = "../reports/report_" + sys.argv[2] + "_" + sys.argv[1] + "_layerslin5_ep250_ba30_" + str(
            n_layers) + "_exp.txt"
