...
# feat = pd.read_excel('../data/mutation_final_selected.xlsx')
    ...
    summ = list()
    stats = []
    summfile = "../reports/summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layersfixed_ep250_ba30_" + type + ".txt"
	...
	#hidden_neurons = np.logspace(np.log10(float(sys.argv[1])), np.log10(5.0), num=n_layers)
        ...
        dir_path = "../Figure/" + "drug_" + str(sys.argv[2]) + "_features_" + str(sys.argv[1]) + "_epoch_" + str(
            training_epochs) + "_nLayersfixed_" + str(n_layers) + "_" + str(sys.argv[6])
        ...
        outputfile = "../reports/report_" + sys.argv[2] + "_" + sys.argv[1] + "_layersfixed_ep250_ba30_" + str(
            n_layers) + "_" + type + ".txt"
        # summ.append('Number of layers: ' + str(n_layers))
        outlist.append('Number layers: ' + str(n_layers))
        summ.append('Number layers: ' + str(n_layers))
        ...
        for k_train, k_test in kf.split(data_kv):
            ...
            with tf.Session(config=config) as sess:  # config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
                sess.run(init)
        ...
        layer_file_name = "Summary_Figure_Layersfixed_" + str(n_layers) + ".png"
        layer_save_path = os.path.join(dir_path, layer_file_name)
        plt.savefig(layer_save_path, format='png', dpi=1000, bbox_inches='tight')

        layer_file_name = "Summary_Figure_Layersfixed_" + str(n_layers) + ".svg"
        layer_save_path = os.path.join(dir_path, layer_file_name)
