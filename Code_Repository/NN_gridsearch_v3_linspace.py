...
with tf.device('/device:GPU:' + str(gpu)):
    ...
    summ = list()
    stats = []
    summfile = "../reports/summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layersLinSp_ep400_ba30_exp_lr_" + str(learning_rate) + '.txt'
    summfile_figure = "summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layersLinSp_ep400_ba30_exp_lr_" + str(learning_rate) + '.txt'
    ...
    for n_layers in range(int(sys.argv[4]), int(sys.argv[5]) + 1):
        ...
        dir_path = "../Figure/" + "drug_" + str(sys.argv[2]) + "_features_" + str(sys.argv[1]) + "_epoch_" + str(
            training_epochs) + "_nLayers_" + str(n_layers) + "_LinSp_" + str(sys.argv[6]) + 'lr_' + str(learning_rate)
        ...
        s_v = statistics(trainaccs, testaccs, sens, spec)
        train_mean_nlist.append(s_v[0])
        test_mean_nlist.append(s_v[2])
        sens_mean_nlist.append(s_v[4])
        spec_mean_nlist.append(s_v[6])
        total_nlayers.append(n_layers)
        print('Working')
        ...
        graph_plot(313, range(1, len(specifity_dict.keys()) + 1), specifity_dict.values(), 'Specificity')
    ...
    # plotting the figure for Layers

    plt.figure(3)

    graph_plot(211, total_nlayers, train_mean_nlist, 'Train Accuracy')
    graph_plot(211, total_nlayers, test_mean_nlist, 'Test Accuracy')
    graph_plot(211, total_nlayers, sens_mean_nlist, 'Sensitivity')
    graph_plot(211, total_nlayers, spec_mean_nlist, 'Specificity')
    graph_legend(211, 'No. of Layers', 'Values', 'Accuracy values vs No. of Layers')

    path = '../Figure/plot_Layers_vs_accuracy_linSp_drug_' + str(sys.argv[2]) + '_layer_' + str(sys.argv[4] + '_to_') + str(sys.argv[5]) + '.png'
