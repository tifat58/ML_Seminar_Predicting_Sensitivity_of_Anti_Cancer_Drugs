...
with tf.device('/device:GPU:' + str(gpu)):
    ...
    # 8. argument: Best hiddne neurons combination lits. eg.: "80, 60"
    ...
    def multilayer_perceptron(x, weights, biases, n_hidden, drop_prob):
    ...
    summ = list()
    stats = []
    # summfile = "../reports2/hyper_para_tune_summary_" + sys.argv[2] + "_" + sys.argv[1] + "_layerslog5_ep400_ba30_exp_lr_" + str(learning_rate) + '.txt'
    ...
    new_report_file = '156_hyper_para_tune_report_drug_' + sys.argv[2] + '_features_' + sys.argv[1] + '_hlayers_' + sys.argv[4] + '_' + sys.argv[5] + '_part' + sys.argv[7] + '_new700.txt'
    new_summ_file = '156_hyper_para_tune_summary_drug_' + sys.argv[2] + '_features_' + sys.argv[1] + '_hlayers_' + sys.argv[4] + '_' + sys.argv[5] + '_part' + sys.argv[7] + '_new700.txt'
    best_model_file = '156_hyper_para_tune_best_model_drug_' + sys.argv[2] + '_features_' + sys.argv[1] + '_hlayers_' + sys.argv[4] + '_' + sys.argv[5] + '_part' + sys.argv[7] + '_new700.txt'
    ...
    list_learning_range = [0.0001, 0.00001]
    list_epoch = [200]
    list_dropout =[0.4, 0.3]
    ...
    for n_layers in range(int(sys.argv[4]), int(sys.argv[5]) + 1):
        ...
        validcomb = [ast.literal_eval(sys.argv[8])]
        print("Best neuron combi:", validcomb)

        if (n_layers != len(validcomb[0])):
            print("miss-match in hidden layers number calculation")
        ...
        for count_epcoh in list_epoch:
            training_epochs = count_epcoh
            ...
            for count_lr in list_learning_range:
                ...
                for count_drop in list_dropout:
                    ...
                    for hidden_neurons in hidden_list[parts_list[part_number][0]:parts_list[part_number][1]]:
                        # make for loops for each layer, have list input features - 20 steps until 20
                        ...
                        model_name = 'feat_' + sys.argv[1] + '__' + str(n_layers) + '_layer_' + str(hidden_neurons) + '_epoch_' + str(count_epcoh) + '_drop_' + str(count_drop) + '_lr_' + str(count_lr)
                            ...
                            with tf.Session(
                                ...
                                for epoch in range(training_epochs):
                                    ...
                                    for i in range(total_batch):  # print('batch x ', Y_batches[i])
                                        ...
                                        _, c = sess.run([optimizer, cost], feed_dict={x: batch_x, y: batch_y, drop_prob: drop_val})
                                    ...
                                    epoch_test_accuracy = accuracy.eval({x: X_test, y: Y_test, drop_prob:1.0})
                                ...
                                print("Train Accuracy:", accuracy.eval({x: X_train, y: Y_train, drop_prob:drop_val}))
                                train_accuracy = accuracy.eval({x: X_train, y: Y_train, drop_prob:drop_val})
                                ...
                                summ.append('Train Accuracy: ' + str(accuracy.eval({x: X_train, y: Y_train, drop_prob:drop_val})))
                                trainaccs.append(accuracy.eval({x: X_train, y: Y_train}))

                                # Train Sensitivity and Specifity
                                pred_labels = pred_b.eval({x: X_train, y: Y_train, drop_prob:drop_val})
